...
void ParticleManager::grid_search()
{
    ...
    for (Particle &it : this->particles)
    {
        ...
        if (x < 0)
            x = 0;
        if (x >= WINDOW_WIDTH / MAX_RADIUS)
            x = WINDOW_WIDTH / MAX_RADIUS - 1;
        if (y < 0)
            y = 0;
        if (y >= WINDOW_HEIGHT / MAX_RADIUS)
            y = WINDOW_HEIGHT / MAX_RADIUS - 1;
...
void ParticleManager::generate_particles(int number)
{
    for (int i = 0; i < number; i++)
    {
        ...
        if (gravity)
            p.acceleration = sf::Vector2f(0, GRAVITY);
        if (friction)
        ...
        for (int j = 0; j < this->grid->at(i)->size(); j++)
        {
            ...
            for (int dx = -1; dx <= 1; dx++)
            {
                for (int dy = -1; dy <= 1; dy++)
                {
                    ...
                    if (i + dx < 0 || i + dx >= this->grid->size() || j + dy < 0 || j + dy >= this->grid->at(i)->size())
                        continue;
